        ...
        <div class="card-body p-4">
            <div class="flex flex-col md:flex-row gap-4">
                ...
                @if($search || $categoryFilter)
                    <button wire:click="clearFilters" class="btn btn-error btn-outline">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="card bg-base-100 shadow-md hover:shadow-2xl transition-all duration-300 group flex flex-col h-full">
                    ...
                    <div class="card-body p-5 flex-1 flex flex-col">
                        <div class="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-base-content/50 mb-2">
                            <span class="text-warning">{{ $post->category->name }}</span>
                            <span>•</span>
                            <span>{{ $post->published_at?->format('M d, Y') ?? $post->created_at->format('M d, Y') }}</span>
                        </div>
                        
                        <h3 class="card-title text-lg font-bold leading-tight mb-2 group-hover:text-warning transition-colors">
                            {{ $post->title }}
                        </h3>
    ...
    @else
        <div class="card bg-base-100 shadow-xl border-2 border-dashed border-base-300">
            <div class="card-body text-center py-16">
                ...
                <h3 class="text-2xl font-bold mb-2">No approved articles yet</h3>
                <p class="text-base-content/60 mb-8 max-w-md mx-auto">There are no published articles from other users at the moment.</p>
            </div>
        </div>
    @endif
        ...
        <div class="modal modal-open">
            <div class="modal-box w-11/12 max-w-4xl p-0 overflow-hidden">
                    ...
                    <div class="max-w-3xl mx-auto">
                        ...
                        <div class="flex flex-wrap items-center gap-4 mb-6 text-sm">
                            ...
                            <span class="text-base-content/60 flex items-center gap-1">
                                ...
                                {{ $selectedPost->published_at?->format('F j, Y') ?? $selectedPost->created_at->format('F j, Y') }}
                            </span>
                            <span class="text-base-content/60 flex items-center gap-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                ...
                <div class="bg-base-200 px-6 py-4 flex justify-end gap-2 border-t border-base-300">
                    ...
                    <a href="{{ route('news.show', $selectedPost->slug) }}" class="btn btn-warning" target="_blank">
    ...
    @if($showEditModal)
        <div class="modal modal-open">
            <div class="modal-box">
                ...
                <p class="py-4">You are about to edit an article published by another community member. This article is live and changes will be visible immediately. Are you sure you want to proceed?</p>
        ...
        <div class="modal modal-open">
            <div class="modal-box">
                <h3 class="font-bold text-lg text-error flex items-center gap-2">
                    ...
                    Delete Community Article?
                </h3>
                <p class="py-4">You are about to permanently delete an article published by another community member. This action cannot be undone and the article will be removed from the website. Are you sure?</p>
                <div class="modal-action">
